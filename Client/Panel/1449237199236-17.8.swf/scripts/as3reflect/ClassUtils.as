package as3reflect
{
   ...
   public class ClassUtils
   {
      ...
      public static function getFullyQualifiedImplementedInterfaceNames(param1:Class, param2:Boolean = false) : Array
      {
         ...
         var _loc8_:XMLList;
         var _loc7_:XML;
         if(_loc8_ = (_loc7_ = MetadataUtils.getFromObject(param1)).factory.implementsInterface)
         {
            _loc3_ = _loc8_.length();
            _loc4_ = 0;
            while(_loc4_ < _loc3_)
            {
               _loc5_ = _loc8_[_loc4_].@type.toString();
               if(param2)
               {
                  _loc5_ = convertFullyQualifiedName(_loc5_);
               }
               _loc6_.push(_loc5_);
               _loc4_++;
            }
